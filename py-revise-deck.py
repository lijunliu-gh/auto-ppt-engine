...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Revise an existing deck through the Python smart layer.")
    parser.add_argument("--deck", default=str(DEFAULT_DECK_INPUT), help="Existing deck JSON file to revise")
    parser.add_argument("--prompt", help="Revision request in natural language")
    parser.add_argument("--prompt-file", help="Read the revision request from a text file")
    parser.add_argument("--out-json", default=str(DEFAULT_JSON_OUTPUT), help="Where to save the revised deck JSON")
    parser.add_argument("--out-pptx", default=str(DEFAULT_PPTX_OUTPUT), help="Where to save the revised PPTX file")
    parser.add_argument("--context-file", action="append", default=[], help="Extra context material to include in revision planning")
    parser.add_argument("--source", action="append", default=[], help="Source file path or URL to include in revision planning")
    parser.add_argument("--research", action="store_true", help="Attempt optional web research when TAVILY_API_KEY is configured")
    parser.add_argument("--mock", action="store_true", help="Skip model calls and use the local heuristic reviser")
    args = parser.parse_args()
    if args.prompt_file:
        args.prompt = read_text_file(resolve_path(args.prompt_file))
    if not args.prompt or not args.prompt.strip():
        parser.error("A revision prompt is required. Use --prompt or --prompt-file.")
    return args


def main() -> None:
    args = parse_args()
    existing_deck = json.loads(read_text_file(resolve_path(args.deck)))
